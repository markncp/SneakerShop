...
class PaymentFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        totalPrice = bundle?.get("totalPrice").toString()

        permission()
        txtidorder = root.findViewById(R.id.txtidorder)
        txtprice = root.findViewById(R.id.txtprice)
        editTextComment = root.findViewById(R.id.editComment)
        ...
        imageViewSlipSelection?.setOnClickListener{
            ...
            builder1.setMessage("ท่านตอ้งการเลือกรูปภาพที่มีอยแู่ ลว้ หรือถ่ายภาพใหม่?")
...
//Set permission to open camera and access a directory
    ...
    private fun payment()
    {
        ...
        val formBody: RequestBody = MultipartBody.Builder()
                ...
                .addFormDataPart("orderID",txtidorder?.text.toString())
                .addFormDataPart("price",totalPrice.toString())
            ...
            if (response.isSuccessful) {
                Log.d("taggg",response.body!!.string())
                 /*try {
                    ...
                    if (data.length() > 0) { Toast.makeText(requireContext(), "ทำการชำระเงินสำเร็จแล้ว",
...
//show image
        ...
        if (requestCode == 200 && resultCode == Activity.RESULT_OK) {
            ...
            try {
                ...
                imageBitmap = resolveRotateImage(imageBitmap, imageFilePath!!)
//show image
...
// "RECREATE" THE NEW BITMAP
    ...
    private fun updatestatus1()
    {
        var url: String =
                getString(R.string.root_url) + getString(R.string.StatusOrder1_url) + orderID
        val okHttpClient = OkHttpClient()
